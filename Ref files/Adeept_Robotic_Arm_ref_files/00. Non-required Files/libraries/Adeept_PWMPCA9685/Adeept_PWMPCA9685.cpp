...
Adeept_PWMPCA9685::Adeept_PWMPCA9685(uint8_t addr) {
  _i2caddr = addr;
}

void Adeept_PWMPCA9685::begin(void) {
 WIRE.begin();
 reset();
}


void Adeept_PWMPCA9685::reset(void) {
 write8(PCA9685_MODE1, 0x0);
}

void Adeept_PWMPCA9685::setPWMFreq(float freq) {
...
void Adeept_PWMPCA9685::setPWM(uint8_t num, uint16_t on, uint16_t off) {
...
void Adeept_PWMPCA9685::setPin(uint8_t num, uint16_t val, bool invert)
{
...
uint8_t Adeept_PWMPCA9685::read8(uint8_t addr) {
  WIRE.beginTransmission(_i2caddr);
  WIRE.write(addr);
  WIRE.endTransmission();
...
void Adeept_PWMPCA9685::write8(uint8_t addr, uint8_t d) {
  WIRE.beginTransmission(_i2caddr);
  WIRE.write(addr);
  WIRE.write(d);
  WIRE.endTransmission();
